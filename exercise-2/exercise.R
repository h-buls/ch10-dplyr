...
two_wheel_drive_20_miles_cty <- filter(vehicles, vehicles$drive == "2-Wheel Drive", cty > 20)

# Of the above vehicles, what is the vehicle ID of the vehicle with the worst 
# hwy mpg?
# Hint: filter for the worst vehicle, then select its ID.

worst_vehicle <- filter(two_wheel_drive_20_miles_cty, hwy == min (hwy))
worst_vehicle_id <- select(worst_vehicle, id)
